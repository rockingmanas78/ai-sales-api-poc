...
  } catch (error) {
    console.error(
      `[parseAndSeedCsvService] FAILED for jobId ${jobId}:`,
      error
    );
    throw error;
  }
...
  try {
    console.log(`[configureCsvImportService] Configuring job: ${jobId}`);
...
  try {
    console.log(`[startCsvImportService] Starting job: ${jobId}`);
...
    if (!job) {
      console.error(
        `[getCsvImportJobStatusService] Job not found: ${jobId}`
      );
...
  ) {
    console.warn(
      `[validateRowData] Invalid LinkedIn URL: ${linkedInUrl} (Type: ${typeof linkedInUrl})`
    );
...
  try {
    const batchSize = 5;
    console.log(`[processCsvImportJob] Starting batch for job: ${jobId}`);
...
    // Fetch job counters (not strictly needed here, but good for context if debugging)
...
    console.log(
      `[processCsvImportJob] Fetched ${rows.length} rows for batch.`
    );
...
        // "SKIPPED" (for DRY_RUN) doesn't increment a counter, which is fine
      } catch (error) {
        failedCount++;
        console.error(
...
    );
    console.error("[processCsvImportJob] Stack Trace:", error.stack);
    // Don't re-throw here, as it would crash the worker.
    // The job will remain in "PROCESSING" state and will be picked up again
    // by the fetchJobDetails logic (which handles re-queuing or failure).
  }
};
...
      where: {
        tenantId: job.tenantId,
        // This logic might need adjustment based on your exact domain+name definition
        // Assuming domain is stored in contactEmail array and name in companyName
...
        console.log(`[processCsvJobs] Worker: Processing job ${job.id}`);
        // Start the job processing (it will run asynchronously via setImmediate)
...
    }
  } catch (error) {
    console.error("[processCsvJobs] CRITICAL error in worker:", error);
    // Do not re-throw, allow the worker to be called again later
  }
};
