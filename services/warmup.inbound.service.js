...
function extractEmails(input) {
  ...
  return [...new Set(output.map(safeLowercaseEmail))];
}

function cleanMessageId(value) {
  if (!value) return null;
  let finalValue = Array.isArray(value) ? value[value.length - 1] : value;
  finalValue = String(finalValue).trim();
  if (finalValue.startsWith("<") && finalValue.endsWith(">")) finalValue = finalValue.slice(1, -1);
...
function getHeader(headers, key) {
  ...
  for (const [headerKey, headerValue] of Object.entries(headers || {})) {
    if (String(headerKey || "").toLowerCase() === target) return headerValue;
  }
  return undefined;
}
...
function hasHeader(headers, key) {
  ...
  return headerValue !== undefined && headerValue !== null && String(headerValue).trim() !== "";
}

function findWarmupReplyToAddress(emails) {
  const list = asArray(emails).map(safeLowercaseEmail);
  return list.find((emailAddress) => emailAddress.startsWith("wreply+")) || null;
}
...
function getMaxTurnsPerThreadPerDay() {
  const rawValue = Number(process.env.WARMUP_MAX_TURNS_PER_THREAD_PER_DAY || 6);
  return Number.isFinite(rawValue) && rawValue > 0 ? rawValue : 6;
}

/**
 * Increment WarmupInboxDailyCounter.sent for a warmup inbox.
 */
async function incrementWarmupInboxSentCounter({ warmupInboxId, tenantId }) {
  if (!warmupInboxId) return;

  const todayDateUtc = getStartOfTodayUtcDateOnly();
...
async function generateWarmupReplyText({ latestEmailText }) {
  const fallbackText = "Got it — thanks for the update!";
  const endpoint = process.env.AI_SERVICE_ENDPOINT;
  const path = process.env.AI_WARMUP_REPLY_PATH || "/api/warmup/reply";
  const secret = process.env.AI_INTERNAL_SECRET || process.env.WEBHOOK_SECRET;

  if (!endpoint) return fallbackText;
...
export async function processWarmupInboundEvent(eventPayload) {
  ...
  // A) inbound to warmup inbox address
  ...
  // B) inbound to reply-domain address (wreply+token@warmup.domain)
...
async function handleInboundToWarmupInbox({
  ...
  const sendResult = await sendRawEmailWithHeaders({
  ...
  const outboundProviderMessageId = sendResult?.MessageId;

  if (outboundProviderMessageId) {
    const outboundMessage = await prisma.warmupMessage.create({
      data: {
        tenantId,
        threadId: thread.id,
        direction: "OUTBOUND",
    ...
    await prisma.warmupMessageEvent.create({
      data: {
        tenantId,
        warmupMessageId: outboundMessage.id,
        providerMessageId: outboundProviderMessageId,
        eventType: "SEND",
        occurredAt: new Date(),
        snsMessageId: `warmup-inbox-out:${outboundProviderMessageId}`,
        payload: { auto: true, side: "INBOX" },
      },
    });

    // ✅ NEW: increment inbox "sent" counter (for this warmup inbox)
    await incrementWarmupInboxSentCounter({
      warmupInboxId: warmupInbox.id,
      tenantId,
    });
  }
}
...
async function handleInboundToReplyDomain({
  ...
  const maxTurnsPerDay = getMaxTurnsPerThreadPerDay();
  ...
  if (turnsToday >= maxTurnsPerDay) return;
  ...
  const outboundProviderMessageId = sendResult?.MessageId;
  if (!outboundProviderMessageId) return;
  ...
  await prisma.warmupMessageEvent.create({
    data: {
      ...
      eventType: "SEND",
      occurredAt: new Date(),
      snsMessageId: `warmup-reply-out:${outboundProviderMessageId}`,
