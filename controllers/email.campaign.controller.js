  ...
  try {
    ...
    if (
      ![
        ...
        "LEAD_NURTURING",
        "RE_ENGAGEMENT",
      ].includes(campaignTypeValue)
    ) {
      return res.status(400).json({
        error: `campaignType must be one of: ${[
          "COLD_OUTREACH",
          "FOLLOW_UP_SEQUENCE",
          "LEAD_NURTURING",
          "RE_ENGAGEMENT",
...
export const getCampaignById = async (req, res) => {
  try {
    ...
    if (!campaign) {
      return res
        .status(404)
        .json({ error: "Campaign not found or does not belong to tenant" });
    }
...
export const getCampaignDashboard = async (req, res) => {
  ...
  try {
    ...
    const campaignCards = campaigns.map((campaign) => {
      ...
      const opened = campaign.logs.filter(
        (log) => log.status === "OPENED"
      ).length;
      const replied = campaign.logs.filter(
        (log) => log.status === "REPLIED"
      ).length;
    ...
    const avgOpenRate = totalEmailsSent
      ? Math.round((totalEmailsOpened / totalEmailsSent) * 100)
      : 0;
    const avgReplyRate = totalEmailsSent
      ? Math.round((totalEmailsReplied / totalEmailsSent) * 100)
      : 0;
