...
// export const emailQueue = new Queue("emailQueue", { connection: redisConn });

// export async function createBulkEmailJob(req, res) {
//     try {
//         const { tenantId, templateId, leadIds, rateLimit } = req.body;
//         if (!tenantId || !templateId || !Array.isArray(leadIds) || !rateLimit?.count || !rateLimit.interval) {
//             return res.status(400).json({ error: "tenantId, templateId, leadIds, rateLimit.count, rateLimit.interval are required" });
//         }
//         const tenant = await prisma.tenant.findFirst({ where: { id: tenantId, deletedAt: null } });
//         if (!tenant) return res.status(404).json({ error: "Tenant not found" });

//         const template = await prisma.emailTemplate.findFirst({ where: { id: templateId, tenantId, deletedAt: null } });
//         if (!template) return res.status(404).json({ error: "Template not found" });

//         const leads = await prisma.lead.findMany({ where: { id: { in: leadIds }, tenantId, deletedAt: null } });
//         if (leads.length !== leadIds.length) {
//             return res.status(400).json({ error: "Some leads missing or not in tenant" });
//         }
...
//                 rateInterval: rateLimit.interval,
//                 nextProcessTime: new Date(),
//                 jobLeads: {
...
//             include : {jobLeads: true},
//         });
//         await emailQueue.add("processJob", { jobId: job.id }, { delay: 0 });
//         res.status(201).json(job);

//     }  catch (error) {
//     console.error("bulk email job creation error ", error);
...
//       where: { id: jobId, tenantId },
//       include: {
//         jobLeads: { include: { lead: true } },
//       },
//     });
//     if (!job) return res.status(404).json({ error: "Job not found" });
//     res.json(job);
//   } catch (error) {
//     console.error("error finding bulk email job ", error);
//     return res.status(500).send("Internal server error");
//   }
// }