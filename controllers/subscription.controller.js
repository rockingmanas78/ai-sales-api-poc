// import { quotePlanWithTax } from "../services/payment.service";
import {
  startPaymentService,
  updateSubscriptionService as updateSubService,
  cancelSubscriptionService,
  getSubscriptionNameService,
  getSubscriptionChargesService,
...
export const updateSubscription = async (req, res) => {
  ...
  if (!tenantId || !versionId) {
    return res
      .status(400)
      .json({ message: "tenantId and versionId are required" });
  }
  ...
  try {
    ...
    if (!planVersion?.Plan) {
      return res
        .status(400)
        .json({ message: "Plan relation missing in PlanVersion" });
    }
    ...
    if (requiresPayment) {
      if (
        typeof planVersion.basePriceCents !== "number" ||
        planVersion.basePriceCents < 0
      ) {
      ...
      try {
        ...
        return res
          .status(502)
          .json({ message: "Failed to initiate payment process" });
      }
      ...
      if (!phonePeMerchantTransactionId || !phonePeRedirectUrl) {
        return res
          .status(500)
          .json({ message: "PhonePe did not return valid transaction data" });
      }
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to create/update subscription" });
    }
      ...
      try {
        await prisma.paymentTransaction.create({
          data: {
            ...
            providerId: merchantTransactionId,
            amount,
            status: "INITIATED",
          },
        });
        ...
        return res
          .status(500)
          .json({ message: "Failed to record payment transaction" });
      }
    }

    return res.status(200).json({
      message: requiresPayment
        ? "Subscription update initiated. Awaiting payment confirmation."
...
export const verifyPhonePePaymentStatus = async (req, res) => {
  ...
  if (!orderId || typeof orderId !== "string") {
    return res
      .status(400)
      .json({ message: "Order ID is required and must be a string" });
  }
  ...
  try {
    ...
    if (!subscriptionId) {
      return res.status(400).json({
        message: "Subscription not found for this payment transaction",
      });
    }
    ...
    if (subscription && subscription.status === "ACTIVE") {
      ...
      if (currentSub) {
        ...
        if (
          ...
          !Object.values(PlanType).includes(planCode)
        ) {
          console.log(`Invalid Plan Code ${planCode} `);
          return res.status(500).json({ message: "Invalid Plan Code" });
        }
        ...
        try {
          ...
          return res
            .status(500)
            .json({ message: "Failed to update tenant plan" });
        }
      }
    }
...
export async function startPayment(req, res) {
  try {
    const result = await startPaymentService(req.body);
...
export async function updateSubscription(req, res) {
  const result = await updateSubService(req.body);
...
export async function cancelSubscription(req, res) {
  ...
  try {
    const result = await cancelSubscriptionService({ tenantId });
    res.status(200).json(result);
  } catch (e) {
...
export async function getSubscriptionName(req, res) {
  try {
    // Expect tenantId in params
    const { tenantId } = req.params;
    if (!tenantId) {
      console.log("Missing tenantId param");
      return res.status(400).json({ error: "tenantId is required" });
    }
...
export async function getSubscriptionCharges(req, res) {
  const { tenantId } = req.params;
  const result = await getSubscriptionChargesService(tenantId);
  res.status(200).json(result);
}

export async function getPaymentStatus(req, res) {
  const { paymentId } = req.params;
  const result = await getPaymentStatusService(paymentId);
  res.status(200).json(result);
}
